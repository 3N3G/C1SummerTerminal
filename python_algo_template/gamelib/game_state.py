...
class GameState:
    ...
    def can_spawn(self, unit_type, location, num=1, verbose=True):
        ...
        if not self.game_map.in_arena_bounds(location):
            if self.enable_warnings:
                if verbose: self.warn("Could not spawn {} at location {}. Location invalid.".format(unit_type, location))
            return False
        ...
        if self.enable_warnings:
            ...
            if len(fail_reason) > 0:
                if verbose: self.warn("Could not spawn {} at location {}.{}".format(unit_type, location, fail_reason))
    ...
    def unsafe_spawn(self, unit_type, location, num=1, player=0):
        if unit_type not in ALL_UNITS:
            self._invalid_unit(unit_type)
            return
        if num < 1 or not location:
            self.warn("Attempted to spawn fewer than one units! ({})".format(num))
            return
        for i in range(num):
            self.game_map.add_unit(unit_type, location, player)
    ...
    def unsafe_upgrade(self, location):
        x, y = map(int, location)
        for unit in self.game_map[x,y]:
            if unit.stationary:
                unit.upgrade()
    ...
    def unsafe_pathfind(self, start_location, target_edge=None, initial_move_direction=0):
        if self.contains_stationary_unit(start_location):
            self.warn("Attempted to perform pathing from blocked starting location {}".format(start_location))
            return
        ...
        return self._shortest_path_finder.unsafe_pathfind(start_location, end_points, self, initial_move_direction)
    ...
    def contains_stationary_nonsupport_unit(self, location, force_turrets):
        """Check if a location is blocked, return structures unit if it is
        ...
        x, y = map(int, location)
        for unit in self.game_map[x,y]:
            if unit.stationary and unit.unit_type != "EF" and (force_turrets or (unit.unit_type != "DF" or unit.upgraded)):
                return unit
        return False
